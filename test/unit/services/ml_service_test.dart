...
// FIXED VERSION - Added test client injection support
...
class MLService {
  ...
  // ADDED: Static test client for dependency injection
  static http.Client? _testClient;

  // ADDED: Method to set test client
  static void setTestClient(http.Client? client) {
    _testClient = client;
  }

  // ADDED: Get the appropriate client (test or real)
  ...
  static Future<MLRecommendationResponse> searchWorkers({
    ...
    try {
      // MODIFIED: Use _client instead of direct http calls
...
class AIAnalysis {
  ...
  AIAnalysis({required this.servicePredictions});
